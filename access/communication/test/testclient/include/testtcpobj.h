...
public:	
	...
	CTestTcpObj(){m_policy = NULL;};
	virtual ~CTestTcpObj(){printf("~CTestTcpObj\n");};
	...
	{
		//创建一个新的连接对象
		m_policy = NewConnectObject(EMICB_PROTO_Tcp);
	}

	void ProcConnectPolicy(const TICBAddress* ptDevAddress)
	{
		m_policy->SetLocalCfg(NULL,0);
