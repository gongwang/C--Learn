...
#ifndef TICPP_TINYXML_INCLUDED
#define TICPP_TINYXML_INCLUDED
...
{
	TICPP_TIXML_SUCCESS,
	TICPP_TIXML_NO_ATTRIBUTE,
	TICPP_TIXML_WRONG_TYPE
};
...
enum TiXmlEncoding
{
	TICPP_TIXML_ENCODING_UNKNOWN,
	TICPP_TIXML_ENCODING_UTF8,
	TICPP_TIXML_ENCODING_LEGACY
};

const TiXmlEncoding TICPP_TIXML_DEFAULT_ENCODING = TICPP_TIXML_ENCODING_UNKNOWN;
...
protected:
	...
	*/
	...
	{
		assert( p );
		if ( encoding == TICPP_TIXML_ENCODING_UTF8 )
		{
...
public:
	...
	*/
	template< typename T > int QueryValueAttribute( const std::string& name, T* outValue ) const
	{
		...
		if ( !node )
			return TICPP_TIXML_NO_ATTRIBUTE;

		std::stringstream sstream( node->ValueStr() );
		sstream >> *outValue;
		if ( !sstream.fail() )
			return TICPP_TIXML_SUCCESS;
		return TICPP_TIXML_WRONG_TYPE;
	}
	/*
...
public:
	...
	//2013-09-12 wbq Add the patch.
	#ifdef TIXML_USE_STL
	/** Load the document from a string. */
	bool LoadFromString( const std::string& document, TiXmlEncoding encoding );
	#endif

	/** Load the document from a string. */
	bool LoadFromString( const char* document, TiXmlEncoding encoding );
	
	/** Load a file using the current document value.
		Returns true if successful. Will delete any existing
		document data before loading.
	*/
	bool LoadFile( TiXmlEncoding encoding = TICPP_TIXML_DEFAULT_ENCODING );
	/// Save a file using the current document value. Returns true if successful.
	bool SaveFile() const;
	/// Load a file using the given filename. Returns true if successful.
	bool LoadFile( const char * filename, TiXmlEncoding encoding = TICPP_TIXML_DEFAULT_ENCODING );
	...
	*/
	bool LoadFile( FILE*, TiXmlEncoding encoding = TICPP_TIXML_DEFAULT_ENCODING );
	/// Save a file using the given FILE*. Returns true if successful.
	bool SaveFile( FILE* ) const;

	#ifdef TIXML_USE_STL
	bool LoadFile( const std::string& filename, TiXmlEncoding encoding = TICPP_TIXML_DEFAULT_ENCODING )			///< STL std::string version.
	{
...
//		return ( f.buffer && SaveFile( f.buffer ));
	...
	*/
	virtual const char* Parse( const char* p, TiXmlParsingData* data = 0, TiXmlEncoding encoding = TICPP_TIXML_DEFAULT_ENCODING );
