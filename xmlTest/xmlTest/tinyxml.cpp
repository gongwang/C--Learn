...
FILE* TicppTiXmlFOpen( const char* filename, const char* mode )
{
...
TiXmlNode* TiXmlNode::LinkEndChild( TiXmlNode* node )
{
	...
	{
		delete node;
		if ( GetDocument() ) GetDocument()->SetError( TIXML_ERROR_DOCUMENT_TOP_ONLY, 0, 0, TICPP_TIXML_ENCODING_UNKNOWN );
		return 0;
	}
...
void TiXmlElement::SetAttribute( const char * cname, const char * cvalue )
{
	...
	else
	{
		...
		if ( document ) document->SetError( TIXML_ERROR_OUT_OF_MEMORY, 0, 0, TICPP_TIXML_ENCODING_UNKNOWN );
	}
}
...
bool TiXmlDocument::LoadFile( const char* _filename, TiXmlEncoding encoding )
{
	...
	FILE* file = TicppTiXmlFOpen( value.c_str (), "rb" );
	...
	{
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TICPP_TIXML_ENCODING_UNKNOWN );
		return false;
	}
}
...
bool TiXmlDocument::SaveFile( const char * filename ) const
{
	...
	FILE* fp = TicppTiXmlFOpen( filename, "w" );
	if ( fp )
	{
...
std::istream& operator>> (std::istream & in, TiXmlNode & base)
{
	...
	base.Parse( tag.c_str(), 0, TICPP_TIXML_DEFAULT_ENCODING );
	return in;
}
#endif
