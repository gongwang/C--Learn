...
TiXmlNode::TiXmlNode( NodeType _type ) : TiXmlBase()//lint !e69
{
	parent = 0;
	type = _type;
	firstChild = 0;
...
bool TiXmlDocument::LoadFromString( const char* document, TiXmlEncoding encoding )
{
	...
	*/
	...
	strncpy_s(buf,length+1,document,length);
  ...
  // Copy string to temporary FILE buffer
	...
	if(fileBuf == NULL)
		return false;
// 	FILE* fileBuf = NULL;
// 	errno_t err = tmpfile_s(&fileBuf); // case EMFILE error
// 	if(err)
// 		return false;
